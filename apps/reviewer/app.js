...
let artifact=null,videoUrl=null,poseLandmarker=null,cancelled=false;
...
landmarksInput.addEventListener("change",async()=>{try{const file=landmarksInput.files[0];if(!file)return;artifact=JSON.parse(await file.text());assertArtifact(artifact);status.textContent=`${artifact.frames.length} frames importados`;draw();}catch(error){showError(error);}});
...
async function runExtraction(){
  ...
  try{
    ...
    status.textContent=`Concluído: ${frames.length}/${total} frames com pose`;video.currentTime=0;draw();
...
function value(id){return document.querySelector("#"+id).value.trim()}function showError(error){errors.textContent=error instanceof Error?error.message:String(error)}function formatDuration(s){return `${Math.floor(s/60)}:${String(Math.round(s%60)).padStart(2,"0")}`}function download(name,text){const url=URL.createObjectURL(new Blob([text],{type:"application/json"}));const a=document.createElement("a");a.href=url;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
